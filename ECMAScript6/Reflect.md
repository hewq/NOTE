...
> [原创链接](http://es6.ruanyifeng.com/#docs/reflect)

- <a href="#1">概述</a>
- <a href="#2">静态方法</a>
- <a href="#3">实例：使用 Proxy 实现观察者模式</a>
...
（4）`Reflect`对象的方法与`Proxy`对象的方法一一对应，主要是`Proxy`对象的方法，就能在`Reflect`对象上找到对应的方法。这就让`Proxy`对象可以方便地调用对应的`Reflect`方法，完成默认行为，作为修改行为的基础。也就是说，不管`Proxy`怎么修改默认行为，你总可以在`Reflect`上获取默认行为。
...
// 新写法
var theDescriptor = Reflect.getOwnPropertyDescrptor(myObject, 'hidden');
```
...
## <a name="3">实例：使用 Proxy 实现观察者模式</a>
