...
> [原创链接](http://es6.ruanyifeng.com/#docs/regex)
...
第一种情况是，参数字符串，这时第二个参数表示正则表达式的修饰符（flag）。
...
ES6 将这 4 个方法在语言内部全部调用`RegExp`的实例方法，从而做到所有与正则相关的方法，全都定义在`RegExp`对象上。
...
ES6 新增了使用大括号表示 Unicode 字符，这种表示法在正则表达式中必须加上`u`修饰符，才能识别当中的括号，否则会被解读为量词。
...
使用`u`修饰符后，所以量词都会正确识别码点大于`0xFFFF`的 Unicode 字符。
...
```javascript
	...
	console.log(codePointLength(c)); // 2
```
...
上面代码有两个正则表达式，一个使用`g`修饰符，另一个使用`y`修饰符。这两个正则表达式个各执行了两次，第一次执行的时候，两者行为相同，剩余字符串都是`_aa_a`。由于`g`修饰没有位置要求，所有第一次执行会返回结果，而`y`修饰符要求匹配必须从头部开始，所以返回`null`。
...
与`y`修饰符匹配，ES6 的正则实例对象多了`sticky`属性，表示是否设置了`y`修饰符。
...
```javascript
console.log(/\d+(?=%)/.exec('100 % of US presidents have been male')); // null	
console.log(/\d+(?=%)/.exec('100% of US presidents have been male')[0]); // 100
console.log(/\d+(?!%)/.exec('that‘s all 44 of them')[0]); // 44
```
...
```javascript
	console.log(/(?<=\$)\d+/.exec('Benjamin Franklin is on the $100 bill')[0]); // 100
	console.log(/(?<!\$)\d+/.exec('it‘s is worth about €90')[0]); // 90
```
...
首先，后行断言的组匹配，与正常情况下结果不一样的。

```javascript
	console.log(/(?<=(\d+)(\d+))$/.exec('1053')); // [ '', '1', '053', index: 4, input: '1053', groups: undefined ]
	console.log(/^(\d+)(\d+)$/.exec('1053')); // [ '1053', '105', '3', index: 0, input: '1053', groups: undefined ]
```

上面代码中，需要捕捉两个组匹配。没有"后行断言"时，第一个括号是贪婪模式，第二个括号只能捕获一个字符，所以结果是`105`和`3`。而"后行断言"时，由于执行顺序是从右到左，第二个括号是贪婪模式，第一个括号只能捕获一个字符，所以结果是`1`和`053`。

其次，"后行断言"的反斜杆引用，也与通常的顺序相反，必须放在对应的那个括号之前。
...
上面代码中，如果后行断言的反斜杆引用（`\1`）放在括号的后面，就不会得到匹配结果，必须放在前面才可以。因为后行断言是先从左到右扫描，发现匹配以后再回过头，从右到左完成反斜杆引用。
...
组匹配的一个问题是，每一组的匹配含义不容易看出来，而且只能用数字序号（比如`matchObj[1]`）引用，要是组的顺序变了，引用的时候必须修改序号。
...
上面代码中，具名组`as`没有找到匹配，那么`matchObj.groups.as`的属性值就是`undefined`，并且`as`这个键名在`groups`是始终存在的。
...
```javascript
	...
	let date = '2015-01-02'.replace(re, (
...
```javascript
	const RE_TWICE_N = /^(?<word>[a-z]+)!\1$/;
	console.log(RE_TWICE_N.test('abc!abc')); // true
	console.log(RE_TWICE_N.test('abc!ab')); // false
```

这两种引用语法还可以同时使用。

```javascript
	const RE_TWICE_S = /^(?<word>[a-z]+)!\k<word>!\1$/;
	console.log(RE_TWICE_S.test('abc!abc!abc')); // true
	console.log(RE_TWICE_S.test('abc!abc!ab')); // false
```

## <a name="12">String.prototype.matchAll</a>

如果一个这则表达式在字符串里面有多个匹配，现在一般使用`g`修饰符或`y`修饰符，在循环里面逐一取出。
...
[code](https://github.com/hewq/ECMAScript6/blob/master/apps/n05regexpextend/n01.js)